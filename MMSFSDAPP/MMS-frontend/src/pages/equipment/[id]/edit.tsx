...
const EditEquipmentPage = () => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [eqData, basesData, unitsData, soldiersData] = await Promise.all([
        equipmentService.getEquipmentItem(id as string),
      ...
      const extractId = (val: any) => (typeof val === 'object' && val?._id) ? val._id : (val || '');
      formik.setValues({
        name: eqData.name || '',
        serialNumber: eqData.serialNumber || '',
        type: eqData.type || '',
        category: eqData.category || '',
        base: extractId(eqData.base),
        assignedUnit: extractId(eqData.assignedUnit),
        assignedSoldier: extractId(eqData.assignedSoldier),
        status: eqData.status || 'Operational',
        condition: eqData.condition || 'Good',
        quantity: String(eqData.quantity || 1),
        acquisitionDate: fmt(eqData.acquisitionDate),
        lastMaintenanceDate: fmt(eqData.lastMaintenanceDate),
        nextMaintenanceDate: fmt(eqData.nextMaintenanceDate),
        notes: eqData.notes || '',
      });
    } catch (error) {
  ...
  const formik = useFormik({
    initialValues: {
      name: '', serialNumber: '', type: '', category: '', base: '', assignedUnit: '', assignedSoldier: '',
      status: 'Operational', condition: 'Good', quantity: '1',
      acquisitionDate: '', lastMaintenanceDate: '', nextMaintenanceDate: '', notes: '',
    },
    ...
    onSubmit: async (values) => {
      try {
        ...
        await equipmentService.updateEquipment(id as string, submitData);
        addNotification({ type: 'success', title: 'Equipment Updated', message: `${values.name} has been updated successfully.` });
  ...
  return (
    <>
      <Head><title>Edit Equipment | MRMS</title></Head>
      <div className="py-6">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 md:px-8">
          <div className="page-header">
            ...
            <h1 className="page-title">Edit Equipment</h1>
          </div>
              ...
              <form onSubmit={formik.handleSubmit}>
                <div className="form-section">
                  <div className="form-section-header">
                    ...
                    <p className="form-section-desc">Update the basic details and classification of this equipment.</p>
                  </div>
                  <div className="grid grid-cols-1 gap-y-5 gap-x-4 sm:grid-cols-6">
                    ...
                    <div className="sm:col-span-3 form-group">
                      ...
                      <input type="text" name="serialNumber" id="serialNumber" className="form-input"
                    ...
                    <div className="sm:col-span-2 form-group">
                      ...
                      <input type="text" name="category" id="category" className="form-input"
                ...
                <div className="form-footer">
                  <Link href={`/equipment/${id}`} className="btn btn-secondary">Cancel</Link>
